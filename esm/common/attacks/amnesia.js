import { ChooseAttackPrompt, EndTurnEffect, GameError, GameLog, GameMessage, StateUtils, UseAttackEffect, } from '@ptcg/common';
const AMNESIA_MARKER = 'AMNESIA_MARKER_{name}';
export const amnesia = function (self, store, state, effect) {
    ...
    return {
        use: (attackEffect) => {
            ...
            return store.prompt(state, new ChooseAttackPrompt(player.id, GameMessage.CHOOSE_ATTACK_TO_DISABLE, [pokemonCard], {
                allowCancel: true,
            }), attack => {
                if (attack !== null) {
