import { AttackEffect, ChooseAttackPrompt, GameLog, GameMessage, StateUtils, } from '@ptcg/common';
export const metronome = function (self, store, state, effect) {
    return {
        use: (attackEffect) => {
            ...
            return store.prompt(state, new ChooseAttackPrompt(player.id, GameMessage.CHOOSE_ATTACK_TO_COPY, [pokemonCard], {
                allowCancel: true,
            }), result => {
                if (result !== null) {
                    const attack = result;
