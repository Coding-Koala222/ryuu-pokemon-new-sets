import { CardManager, CheckPokemonPlayedTurnEffect, ChooseCardsPrompt, ChoosePokemonPrompt, EvolveEffect, GameError, GameMessage, PlayerType, PokemonCard, SlotType, Stage, SuperType, } from '@ptcg/common';
...
function* playCard(next, store, state, effect) {
    ...
    let cards = [];
    return store.prompt(state, new ChooseCardsPrompt(player.id, GameMessage.CHOOSE_CARD_TO_EVOLVE, player.hand, { superType: SuperType.POKEMON, stage: Stage.STAGE_2 }, { min: 1, max: 1, allowCancel: true, blocked: blocked2 }), selected => {
        cards = selected || [];
        if (cards.length > 0) {
...
export const rareCandy = function (self, store, state, effect) {
    return {
